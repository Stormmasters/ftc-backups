...
public class LeoTeleOp_Alex extends OpMode {
    double LX, LY, RX, sensitivity = 0.5, wristPosition = 0.5, ArmPosition = 0.5;
    ...
    public void init() {
        ...
        Arm.setPosition(ArmPosition);//Initialize Arm Position
    }
    ...
    public void loop() {
        ...
        if (gamepad2.dpad_up && !prevDpadUp) {
            ArmPosition -= 0.001;
        } else if (gamepad2.dpad_down && !prevDpadDown) {
            ArmPosition += 0.001;
        }
        ...
        ArmPosition = Math.max(0.0, Math.min(1.0, ArmPosition));
        ...
        telemetry.addData("Shoulder Pos", ArmPosition);
        telemetry.update();
    }
}
